...
Out_Folder = '../eval_res';
img_folder = fullfile(Out_Folder, 'img');
...
for i = 1:1:total_num
    ...
    if (VISUALIZE_REG || (fail && VISUAL_WRONG))
        ...
        title('Matches')
        img_file = fullfile(img_folder, sprintf('%d_match.png', anc_idx));
        exportgraphics(gcf, img_file, 'BackgroundColor', 'current')
        close;
        ...
        title('Alignment')
        img_file = fullfile(img_folder, sprintf('%d_align.png', anc_idx));
        exportgraphics(gcf, img_file, 'BackgroundColor', 'current')
        close;
        ...
        title('GroundTruth')
        img_file = fullfile(img_folder, sprintf('%d_gt.png', anc_idx));
        exportgraphics(gcf, img_file, 'BackgroundColor', 'current')
        close;

        disp(['exported', sprintf('%d', anc_idx)]);
    end
