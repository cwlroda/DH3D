...
def is_gt_match_2D(queries, ref, distance_thresh=25):
    distances = np.linalg.norm(
        ...
        - np.expand_dims([ref["northing"], ref["easting"]], axis=1),
        axis=0,
    )
...
def compute_tp_fp(ref_descriptors, query_descriptors, gt_matches, *arg, **kwarg):
    ...
    def __init__(
        ...
        **kwarg
        ...
        print(
            tabulate(
                [
                    [self.database_sequences[i], len(self.database_desc[i])]
                    for i in range(len(self.database_sequences))
                ],
        ...
        print(
            tabulate(
                [
                    [self.query_sequences[i], len(self.query_desc[i])]
    ...
    def get_database_pos_desc(self, isquery):
        ...
        for seq in useseq:
            ...
            for pcd in seqinfo:
                ...
                desc = load_descriptor_bin(
                    os.path.join(descdir, pcd_filepath + ext), self.desc_dim
                )
    ...
    def evaluate(self):
        ...
        for ref_ind in range(len(self.database_sequences)):
            ...
            for query_ind in range(len(self.query_sequences)):
                ...
                tp, fp, valid, one_percent = compute_tp_fp(
                    ref_desc, query_desc, gt_matches, max_num_nn=self.max_querynum
                )
                recall = np.mean(tp[valid] > 0, axis=0)
                validoneperc = np.mean(one_percent[valid])
                ret = retrieval_Result(
                    self.database_sequences[ref_ind],
                    self.query_sequences[query_ind],
                    recall,
                    validoneperc,
                )
                results.append(ret)
        ...
        with open(self.savedir, "w") as file:
            ...
            print("\n", file=file)
            print("Avg_recall:", file=file)
            for i, r in enumerate(avg_recall):
                print("{}: {:.4f}".format(i + 1, r), file=file)
            print("\n", file=file)
            print("Avg_one_percent_retrieved:", file=file)
            print("{:.4f}".format(avg_one_percent_retrieved), file=file)
...
if __name__ == "__main__":
    ...
    retrieval = GlobalDesc_eval(
        result_savedir="./",
        desc_dir=res_dir,
        database_file=ref_file,
