...
template <typename Dtype, typename NBtype, int C_N = 256, int C_Dout = 32,
          int C_Din = 64>
...
struct ForwardKernel {
        ...
        for (int o_din = 0; o_din < Din; o_din += C_Din) {
            ...
            if (n < N) {
                ...
                for (int k = 0; k < K && n < N; ++k) {
                    ...
                    // Loop over Din
                    for (int din = 0; din < C_Din && (o_din + din) < Din;
                         ++din) {
                        Dtype fk =
                            d_features[b * Din * N + (o_din + din) * N + nk];
                        Dtype f_0 =
                            d_features[b * Din * N + (o_din + din) * N + n_0];

                        // Loop over partial Dout
                        for (int dout = 0; dout < C_Dout &&
                                           (dout + blockIdx.y * C_Dout) < Dout;
                             ++dout) {
                            Dtype w = s_theta[din * C_Dout + dout];
                            result[dout] +=
                                w * (fk - f_0);  // feature relative distance
                        }
                    }
                }

                for (int dout = 0;
                     dout < C_Dout && (dout + blockIdx.y * C_Dout) < Dout;
                     ++dout) {
    ...
    // theta:		parameters for kernel function             	[Din,
    // Dout]. bias:      parameters for kernel function              [Dout].
    const Dtype* d_theta;
    const Dtype* d_bias;
...
struct BackwardFeatureKernel {
    ...
    void launch(int B) {
        ...
        int shm = (theta_size + bias_size + topdiff_size) * sizeof(Dtype) +
                  (nk_size) * sizeof(int);
            ...
            if (n < N) {
                for (int i_dout = threadIdx.y;
                    ...
                    s_topdiff[i_dout * C_N + i_n] =
                        d_topdiff[b * Dout * N +
                                  (dout_outer * C_Dout + i_dout) * N + n];
            }

            for (int dout_inner = 0; dout_inner < C_Dout &&
                                     (dout_outer * C_Dout + dout_inner) < Dout;
                 ++dout_inner) {
                for (int k = 0; k < K; k++) {
                    __syncthreads();
                    ...
                    if (n < N && din < Din) {
                        ...
                        // atomicAdd(
                        //     &d_features_out[b * Din * N + din * N + s_nk[k *
                        //     C_N + i_n]], value);
                        tensorflow::CudaAtomicAdd(
                            &d_features_out[b * Din * N + din * N +
                                            s_nk[k * C_N + i_n]],
                            value);
                        tensorflow::CudaAtomicAdd(
                            &d_features_out[b * Din * N + din * N +
...
struct ConvPointsetFunctor<GPUDevice, Dtype> {
    void operator()(::tensorflow::OpKernelContext* ctx, const Tensor& features,
                    ...
                    const Tensor& neighborhood, Tensor* output) {
        typedef int NBtype;
        ...
        if (!ctx->eigen_gpu_device().ok()) {
            ctx->SetStatus(
                tensorflow::errors::Internal("ConvPointsetFunctor::forward::"
                                             "ForwardKernel execution failed"));
        }
    }
};
...
struct ConvPointsetGrad<GPUDevice, Dtype> {
    void operator()(::tensorflow::OpKernelContext* ctx, const Tensor& features_,
                    ...
                    Tensor* grad_features_, Tensor* grad_theta_,
                    Tensor* grad_bias_) {
        ...
        const Dtype* features_ptr =
            reinterpret_cast<const Dtype*>(features.data());
        const Dtype* theta_ptr = reinterpret_cast<const Dtype*>(theta.data());
        const Dtype* bias_ptr = reinterpret_cast<const Dtype*>(bias.data());

        const Dtype* topdiff_ptr =
            reinterpret_cast<const Dtype*>(topdiff.data());

        Dtype* grad_features_ptr =
            reinterpret_cast<Dtype*>(grad_features.data());
        ...
        ::tensorflow::CudaLaunchConfig cfg = ::tensorflow::GetCudaLaunchConfig(
            N, ctx->eigen_device<GPUDevice>());
        ...
        if (!ctx->eigen_gpu_device().ok()) {
            ctx->SetStatus(tensorflow::errors::Internal(
                "CUDA: BackwardFeatureKernel Error!\n"));
        }
        ...
        if (!ctx->eigen_gpu_device().ok()) {
            ctx->SetStatus(tensorflow::errors::Internal(
                "CUDA: BackwardThetaKernel Error!\n"));
        }
    }
};
