        ...
        for (int k_ = 0; k_ < K; ++k_) {
            ...
            for (int dout = blockIdx.x * blockDim.x + threadIdx.x; dout < Dout;
                ...
                for (int din = 0; din < Din; ++din) {
                    ...
                    tensorflow::CudaAtomicAdd(&grad_bias[din * Dout + dout],
                                              bias_update);
                    ...
                    for (int dp = 0; dp < Dp; ++dp) {
                        ...
                        tensorflow::CudaAtomicAdd(
                            &grad_theta[dp * Din * Dout + din * Dout + dout],
                            theta_update);
                    ...
                    tensorflow::CudaAtomicAdd(
                        &grad_features[b * Din * N + din * N + self_k],
                        feature_update);
                    // tensorflow::CudaAtomicAdd(&grad_features[b * Din * N +
                    // din * N + self_k], 1);
                }
            }
        }
