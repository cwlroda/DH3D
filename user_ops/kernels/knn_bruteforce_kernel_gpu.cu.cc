...
struct BlockBFKernel {
    ...
    __device__ __forceinline__ void operator()() const {
        ...
        if (N > C_THREADS * C_VPT) {
            printf(
                "<BlockBFKernel> Critical problem!!!!! Not enough resources "
                "spend "
                "for N points!!! %d < %d \n",
                C_THREADS * C_VPT, N);
            return;
        ...
        for (int dpi = tid; dpi < Dp; dpi += blockDim.x) {
            s_point[dpi] = d_data[b * N * Dp + dpi * N + y];  // not aligned
                                                              // with data!
        }
        ...
        typedef cub::BlockRadixSort<Dtype, C_THREADS, C_VPT, NBtype>
            BlockRadixSort;
...
struct KnnBruteforceFunctor<GPUDevice, Dtype, NBtype> {
    void operator()(::tensorflow::OpKernelContext* ctx, const Tensor& positions,
        ...
        if (!ctx->eigen_gpu_device().ok()) {
            ctx->SetStatus(
                tensorflow::errors::Internal("KNNBF execution failed"));
        }
    }
};
